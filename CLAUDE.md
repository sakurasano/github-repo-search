...
./gradlew spotlessCheck          # 整形チェック（CIで実行）
```
...
- `di/` — Hiltモジュール。外部I/O関連のインスタンスをprovideし、interfaceに実装を `@Binds` で結びつける。
...
- コメントは目的・意図の高レベルな説明にとどめる。実装ロジックの言い換え（条件分岐の説明など）や、使用箇所・利用シーンの列挙は書かない。コードやコールサイトを見れば分かること、変更で古くなることを二重に書かない。
- テスト名（日本語のバッククォート関数名）は外から見た振る舞い・仕様で表す。`stateIn` の起動やcollect、対象メソッド名といった実装の詳細を名前に出さない。状態名（`Loading` / `Success` 等）は公開状態の契約なので使ってよい。

## テスト方針

- TDDで実装する。まず期待する振る舞いを表すテストを書いて失敗を確認し、それを通す最小限の実装を加えてから、必要に応じてリファクタする。テストが実装なしで通る偽陽性を防ぐため。
- ViewModelテストは `Dispatchers.Main` をテスト用ディスパッチャに差し替えるJUnitルールを使う（`viewModelScope` がMainを使うため）。`app/src/test` に既存の差し替えルールがある。
- リポジトリはinterfaceとして定義し、テストでは実装ではなくフェイクを注入する。固定結果を返すフェイクと、応答タイミングを手動制御できるフェイク（競合・キャンセル検証用）を使い分ける。並行処理の順序ガード（進行中のcancel・番兵・遅れて返る結果の破棄）は、応答制御フェイクで敵対的な順序を作って検証し、即応答フェイクで悪い順序を再現できなければ応答制御フェイクへ格上げする。Android非依存で状態遷移・キャンセル挙動を検証する。
- 操作をまたいで持ち越す内部状態は、別の操作が完了して作った状態から次の操作を始める経路でも検証する。クリーンな状態からの単発遷移だけで満足しない。複数操作をまたぐ不変条件は、最終状態だけでなくフェイクに記録した要求の列をassertして確かめる。
...
依存はすべて `gradle/libs.versions.toml`（version catalog）で管理。`app/build.gradle.kts` では `libs.xxx` 経由で参照する。ハードコードしないこと。
