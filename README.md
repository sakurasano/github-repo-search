...
- 通信の失敗はデータ層で`DataResult`に吸収し、UI層はライブラリ非依存の結果だけを受け取る。通信手段を差し替えても上位に波及させないため。
- 画面状態は限定した種類（初期 / 読み込み中 / 成功 / 0件 / エラー）に固定する。取りうる状態を型で縛り、表示漏れや不整合を防ぐため。
- 新しい検索は進行中の取得を打ち切ってから始める。遅れて返る古い結果で新しい結果を上書きしないため。
- コルーチンのキャンセル時は確実に処理を止める。画面破棄後の不要な更新やクラッシュを防ぐため。
- 画面ロジックはAndroidに依存させない。通信なしで状態遷移・キャンセルをユニットテストで検証するため。
